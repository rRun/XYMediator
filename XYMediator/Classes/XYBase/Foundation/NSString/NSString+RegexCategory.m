 ...
 *  是否是座机
 */
- (BOOL)isValidTellPhone {
    
//    NSString *postalRegex = @"^(0[0-9]{2,4}/-)?([2-9][0-9]{6,8})+(/-[0-9]{1,4})?$";
    NSString *postalRegex = @"([2-9][0-9]{6,8})";
...
//    return (ret || ret1);
    ...
    /**
     20         * 中国电信：China Telecom
     21         * 2G/3G号段（CDMA2000网络）133、153、180、181、189
                * 4G号段 177
     22         */
    ...
    /**
     ...
     26         * 区号：170
     28         */
    NSString * PH = @"^1(7[0])\\d{8}$";
    
    NSPredicate *regextestmobile = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", MOBILE];
    NSPredicate *regextestcm     = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CM];
    NSPredicate *regextestcu     = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CU];
    NSPredicate *regextestct     = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", CT];
    NSPredicate *regestestph     = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", PH];
    
    if (([regextestmobile evaluateWithObject:self] == YES)
         || ([regextestcm evaluateWithObject:self] == YES)
         || ([regextestct evaluateWithObject:self] == YES)
         || ([regextestcu evaluateWithObject:self] == YES)
         || ([regestestph evaluateWithObject:self] == YES))
    {
        if([regextestcm evaluateWithObject:self] == YES) {
            ...
            NSLog(@"中国联通");
        } else if ([regestestph evaluateWithObject:self] == YES) {
            NSLog(@"虚拟运营商");
        } else {
            NSLog(@"未知");
        }
...
//    NSString *carRegex = @"^[\u4e00-\u9fff]{1}[a-zA-Z]{1}[-][a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-\u9fff]$";
    NSString *carRegex = @"^[\u4e00-\u9fff]{1}[A-Z]{1}[-][A-Z_0-9]{4}[A-Z_0-9_\u4e00-\u9fff]$";
